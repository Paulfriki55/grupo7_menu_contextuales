...
import '../main.dart';
import 'cover_page.dart';  // Importa CoverPage

class DividePage extends ConsumerStatefulWidget {
  const DividePage({super.key});

  @override
  ConsumerState<DividePage> createState() => _DividePageState();
}

class _DividePageState extends ConsumerState<DividePage> {
  ...
  void _performOperation(String num1Text, String num2Text) {
    ...
    try {
      ...
      final divideResult = calculator.executeOperation(
        num1,
        num2,
        calculator.calculator.divide,
      );
      setState(() {
        result = divideResult.toStringAsFixed(2);
      });
    } catch (e) {
      setState(() {
        error = e.toString();
